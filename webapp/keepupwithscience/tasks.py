...
def update_metadata(journal_id, feed_data):
    ...
    """
    journal = journals.get(journal_id)
    journals.update(journal, last_checked = datetime.datetime.fromtimestamp(time.time()))
    journals.update(journal, metadata_update = datetime.datetime.fromtimestamp(time.time()))

    #Some feeds has a update frequency data, lets have Worker be nice and try and follow this.
    journals.update(journal, update_frequency= feed_data.feed.get("sy_updatefrequency", None))
    if not journal.update_frequency:
        logger.debug("{0} has not set a updateFrequency.".format(journal.title))
        journals.update(journal, update_frequency = 2)
